        ...
        for digestmod in (GOST34112012256, gost34112012256):
            self.assertSequenceEqual(
                hmac.new(
                    ...
                    msg=bytes.fromhex("0126bdb87800af214341456563780100"),
                    digestmod=digestmod,
                ).hexdigest(),
...
class TestVectors(TestCase):
    def test_m1(self):
        m = bytes.fromhex("323130393837363534333231303938373635343332313039383736353433323130393837363534333231303938373635343332313039383736353433323130")[::-1]
        self.assertSequenceEqual(
            GOST34112012512(m).digest(),
            bytes.fromhex("486f64c1917879417fef082b3381a4e211c324f074654c38823a7b76f830ad00fa1fbae42b1285c0352f227524bc9ab16254288dd6863dccd5b9f54a1ad0541b")[::-1]
        )
        self.assertSequenceEqual(
            GOST34112012256(m).digest(),
            bytes.fromhex("00557be5e584fd52a449b16b0251d05d27f94ab76cbaa6da890b59d8ef1e159d")[::-1]
        )

    def test_m2(self):
        m = u"Се ветри, Стрибожи внуци, веютъ с моря стрелами на храбрыя плъкы Игоревы".encode("cp1251")
        self.assertSequenceEqual(m, bytes.fromhex("fbe2e5f0eee3c820fbeafaebef20fffbf0e1e0f0f520e0ed20e8ece0ebe5f0f2f120fff0eeec20f120faf2fee5e2202ce8f6f3ede220e8e6eee1e8f0f2d1202ce8f0f2e5e220e5d1")[::-1])
        self.assertSequenceEqual(
            GOST34112012512(m).digest(),
            bytes.fromhex("28fbc9bada033b1460642bdcddb90c3fb3e56c497ccd0f62b8a2ad4935e85f037613966de4ee00531ae60f3b5a47f8dae06915d5f2f194996fcabf2622e6881e")[::-1]
        )
        self.assertSequenceEqual(
            GOST34112012256(m).digest(),
            bytes.fromhex("508f7e553c06501d749a66fc28c6cac0b005746d97537fa85d9e40904efed29d")[::-1]
        )
...
class TestPBKDF2(TestCase):
    ...
    def test_1(self):
        self.assertSequenceEqual(
            pbkdf2(b"password", b"salt", 1, 64).hex(),
    ...
    def test_2(self):
        self.assertSequenceEqual(
            pbkdf2(b"password", b"salt", 2, 64).hex(),
    ...
    def test_3(self):
        self.assertSequenceEqual(
            pbkdf2(b"password", b"salt", 4096, 64).hex(),
    ...
    def test_4(self):
        self.assertSequenceEqual(
            pbkdf2(b"password", b"salt", 16777216, 64).hex(),
    ...
    def test_6(self):
        self.assertSequenceEqual(
            pbkdf2(b"pass\x00word", b"sa\x00lt", 4096, 64).hex(),
