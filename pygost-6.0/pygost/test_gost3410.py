...
class Test341001(TestCase):
    def test_rfc(self):
        ...
        self.assertSequenceEqual(pubX.to_bytes(32, "big"), pub_x)
        self.assertSequenceEqual(pubY.to_bytes(32, "big"), pub_y)
        s = sign(c, prv, digest)
...
class Test34102012(TestCase):
    def test_1(self):
        ...
        prv = int.from_bytes(bytes.fromhex("7A929ADE789BB9BE10ED359DD39A72C11B60961F49397EEE1D19CE9891EC3B28"), "big")
        digest = bytes.fromhex("2DFBC1B372D89A1188C09C52E0EEC61FCE52032AB1022E8E67ECE6672B043EE5")
        rand = bytes.fromhex("77105C9B20BCD3122823C8CF6FCC7B956DE33814E95B7FE64FED924594DCEAB3")
        ...
        self.assertSequenceEqual(signature.hex(), s + r)
    ...
    def test_2(self):
        ...
        prv = int.from_bytes(bytes.fromhex("0BA6048AADAE241BA40936D47756D7C93091A0E8514669700EE7508E508B102072E8123B2200A0563322DAD2827E2714A2636B7BFD18AADFC62967821FA18DD4"), "big")
        digest = bytes.fromhex("3754F3CFACC9E0615C4F4A7C4D8DAB531B09B6F9C170C533A71D147035B0C5917184EE536593F4414339976C647C5D5A407ADEDB1D560C4FC6777D2972075B8C")
    ...
    def test_gcl3(self):
        ...
        """
        p = int.from_bytes(bytes(bytearray((
        ...
        q = int.from_bytes(bytes(bytearray((
            0x45, 0x31, 0xAC, 0xD1, 0xFE, 0x00, 0x23, 0xC7,
            0x55, 0x0D, 0x26, 0x7B, 0x6B, 0x2F, 0xEE, 0x80,
            0x92, 0x2B, 0x14, 0xB2, 0xFF, 0xB9, 0x0F, 0x04,
            0xD4, 0xEB, 0x7C, 0x09, 0xB5, 0xD2, 0xD1, 0x5D,
        ...
        a = int.from_bytes(bytes(bytearray((
        ...
        b = int.from_bytes(bytes(bytearray((
        ...
        x = int.from_bytes(bytes(bytearray((
        ...
        y = int.from_bytes(bytes(bytearray((
        ...
        pubX, pubY = public_key(c, prv)
        self.assertSequenceEqual(pubX.to_bytes(64, "big"), pub_x)
        self.assertSequenceEqual(pubY.to_bytes(64, "big"), pub_y)
        s = sign(c, prv, digest)
        self.assertTrue(verify(c, (pubX, pubY), digest, s))
        self.assertTrue(verify(c, (pubX, pubY), digest, signature))
    ...
    def test_sequence(self):
        ...
        prv = int.from_bytes(urandom(64), "big")
        pubX, pubY = public_key(c, prv_unmarshal(prv_marshal(c, prv)))
        for _ in range(20):
            digest = urandom(64)
            s = sign(c, prv, digest)
...
class TestUVXYConversion(TestCase):
    ...
    def test_curve1(self):
        ...
        u, v = (0x0D, int.from_bytes(bytes.fromhex("60CA1E32AA475B348488C38FAB07649CE7EF8DBE87F22E81F92B2592DBA300E7"), "big"))
    ...
    def test_curve2(self):
        ...
        u, v = (0x12, int.from_bytes(bytes.fromhex("469AF79D1FB1F5E16B99592B77A01E2A0FDFB0D01794368D9A56117F7B38669522DD4B650CF789EEBF068C5D139732F0905622C04B2BAAE7600303EE73001A3D"), "big"))
        self.assertEqual(uv2xy(c, u, v), (c.x, c.y))
        self.assertEqual(xy2uv(c, c.x, c.y), (u, v))
